...
public class AddCommandParserTest {
    ...
    public void parse_optionalFieldsMissing_success() {
        ...
        // missing email
        Person expectedPersonNoEmail = new PersonBuilder(AMY).withTags().withoutEmail().build();
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY + ADDRESS_DESC_AMY,
                new AddCommand(expectedPersonNoEmail));

        // missing address
        Person expectedPersonNoAddress = new PersonBuilder(AMY).withTags().withoutAddress().build();
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY + EMAIL_DESC_AMY,
                new AddCommand(expectedPersonNoAddress));

        // missing both email and address
        Person expectedPersonMinimal = new PersonBuilder(AMY).withTags().withoutEmail().withoutAddress().build();
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY,
                new AddCommand(expectedPersonMinimal));

        // missing photo
        Person expectedPersonNoPhoto = new PersonBuilder(AMY).withTags().withoutPhoto().build();
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY + ADDRESS_DESC_AMY + EMAIL_DESC_AMY,
                new AddCommand(expectedPersonNoPhoto));

        // missing email, address and photo
        Person expectedPersonMoreMinimal = new PersonBuilder(AMY).withTags().withoutEmail()
                .withoutAddress().withoutPhoto().build();
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY,
                new AddCommand(expectedPersonMoreMinimal));
    }
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, " n/Alice <edit new> p/78962734",
                ParserUtil.MESSAGE_RESERVED_EDIT_SEGMENT_DELIMITER);

        // reserved edit delimiter token in address
        assertParseFailure(parser, NAME_DESC_BOB + PHONE_DESC_BOB + " a/Block 50 <edit new> West Coast",
                ParserUtil.MESSAGE_RESERVED_EDIT_SEGMENT_DELIMITER);
    }

    @Test
    public void parse_optionalPhotoPresent_success() {
        Person expectedPerson = new PersonBuilder(AMY).withTags()
                .withTags(VALID_TAG_FRIEND)
                .withPhoto(VALID_PHOTO)
                .build();

        assertParseSuccess(parser, PREAMBLE_WHITESPACE + NAME_DESC_AMY + PHONE_DESC_AMY + EMAIL_DESC_AMY
            + ADDRESS_DESC_AMY + TAG_DESC_FRIEND + PHOTO_DESC, new AddCommand(expectedPerson));
    }
}
